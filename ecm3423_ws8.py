...
class ExeterScene(Scene):
    def __init__(self):
        ...
        self.add_models_list(
            [DrawModelFromMesh(scene=self, M=scaleMatrix([0.5,0.5,0.5]), mesh=mesh, shader=FlatShader()) for mesh in meshes]
        )

        table = load_obj_file('models/quad_table.obj')
        self.table = [DrawModelFromMesh(scene=self, M=translationMatrix([0, -3, +0]), mesh=mesh, shader=FlatShader()) for mesh in table]

        bunny = load_obj_file('models/bunny_world.obj')
        self.bunny = DrawModelFromMesh(scene=self, M=np.matmul(translationMatrix([0,+1,0]), scaleMatrix([0.5,0.5,0.5])), mesh=bunny[0], shader=FlatShader())

        box = load_obj_file('models/fluid_border.obj')
        self.box = [DrawModelFromMesh(scene=self, M=translationMatrix([0,+1,0]), mesh=mesh, shader=FlatShader()) for mesh in box]

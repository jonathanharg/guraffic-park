import numpy as np
from matutils import translationMatrix, scaleMatrix

class Entity:
    def __init__(self, position: tuple[float, float, float] = (0.0, 0.0, 0.0), scale: float = 1.0, rotation = None) -> None:
    ...
    def y_rot(self):
        return np.arctan2(-self.rotation[0,2], np.sqrt(self.rotation[1,2]**2 + self.rotation[2,2]**2))
    ...
    def get_pose_matrix(self):
        return np.matmul(np.matmul(self.get_translation_matrix(), self.rotation), self.get_scale_matrix())

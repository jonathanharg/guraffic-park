...
class ExeterScene(Scene):
    def __init__(self):
        ...
        self.light = LightSource(self, position=[3.0, 4.0, -3.0])
        ...
        self.add_models_list(
            [
                DrawModelFromMesh(
                    scene=self,
                    M=np.matmul(
                        translationMatrix([0, -1, 0]), scaleMatrix([0.5, 0.5, 0.5])
                    ),
                    mesh=mesh,
        ...
        self.show_light = DrawModelFromMesh(
            scene=self,
            M=poseMatrix(position=self.light.position, scale=0.2),
            mesh=Sphere(material=Material(Ka=[10, 10, 10])),
            shader=FlatShader(),
        )
        ...
        self.bunny = DrawModelFromMesh(
            scene=self,
            M=np.matmul(translationMatrix([0, 0, 0]), scaleMatrix([0.5, 0.5, 0.5])),
            mesh=bunny[0],
            shader=EnvironmentShader(map=self.environment),
        )
